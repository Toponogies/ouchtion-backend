...
userModel.getPoint = async function (user_id) {
	const row1 = await db('products')
		.join('rates', 'rates.product_id', 'products.product_id')
		.sum('rate as sum_like')
		.whereRaw(
			`((products.buyer_id= ${user_id} and type = 'SELLER-BUYER') or (products.seller_id= ${user_id} and type = 'BUYER-SELLER')) and rate > 0`
		);
	const row2 = await db('products')
		.join('rates', 'rates.product_id', 'products.product_id')
		.count('rate as count_rate')
		.whereRaw(
			`(products.buyer_id= ${user_id} and type = 'SELLER-BUYER') or (products.seller_id= ${user_id} and type = 'BUYER-SELLER')`
		);
